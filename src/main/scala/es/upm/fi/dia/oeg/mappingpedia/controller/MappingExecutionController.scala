                                ...
                                )
{
  ...
  def executeMappingWithFuture(
    ...
    val organizationId = mappingExecution.organizationId
    ...
    val f = Future {
      ...
      val mdHash = if (mappingExecution.mdHash == null && mdDownloadURL != null ) {
        MappingPediaUtility.calculateHash(mdDownloadURL, "UTF-8");
      } else {
        mappingExecution.mdHash
      }
      ...
      if(cacheExecutionURL == null || cacheExecutionURL.results.isEmpty || !useCache) {
        ...
        val mappedClasses:String = try {
          this.mappingDocumentController.findMappedClassesByMappingDocumentId(
            mappingExecution.mdId).results.mkString(",");
        ...
        val mappingExecutionDirectory = s"executions/$organizationId/$datasetId/${mappingExecution.mdId}";
        ...
        val ckanAddResourceResponse = try {
          if(MappingPediaEngine.mappingpediaProperties.ckanEnable && pStoreToCKAN) {
            ...
            val mapTextBody:Map[String, String] = Map(
              MappingPediaConstant.CKAN_RESOURCE_ORIGINAL_DATASET_DISTRIBUTION_DOWNLOAD_URL -> mappingExecution.unannotatedDistributionsDownloadUrls.mkString(",")
              ...
              , MappingPediaConstant.CKAN_RESOURCE_ORIGINAL_DISTRIBUTION_CKAN_ID -> mappingExecution.ckanResourcesIds.mkString(",")
            )
  ...
  */
  ...
  def generateManifestFile(mappingExecutionResult:AnnotatedDistribution
    ...
    try {
      ...
      val mappingDocumentHash = if(pMdHash == null) { "" } else { pMdHash }
